...
- Status auto-syncs to Google Sheet when changed (if OAuth Sheets path is configured)
...
- **Gemini**: [aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey) — free tier (~20 req/day on `gemini-2.5-flash`). Check your real limits at [aistudio.google.com/rate-limit](https://aistudio.google.com/rate-limit)
- **Groq**: [console.groq.com/keys](https://console.groq.com/keys) — free tier, more generous daily limit, hosts Llama models
...
- **Adzuna**: [developer.adzuna.com](https://developer.adzuna.com) — free, good India coverage
- **JSearch via RapidAPI**: [rapidapi.com/.../jsearch](https://rapidapi.com/letscrape-6bRBa3QguO5/api/jsearch) — free tier
...
`JC_JOBID` `JC_TITLE` `JC_COMPANY` `JC_LOCATION` `JC_SALARY` `JC_SOURCE` `JC_URL` `JC_FITSCORE` `JC_MATCHING` `JC_MISSING` `JC_STATUS`

**Analysis Form** (click "Make a copy" in Settings, or 3 questions):
`JC_JOBID` `JC_SUGGESTIONS` `JC_COVERLETTER`

Job ID is a stable hash — use VLOOKUP on it to join both sheets.

After copying each form: link it to a spreadsheet (Responses → Sheets icon), go to ⋮ → Pre-fill form, fill in every placeholder with its `JC_...` text, Get link, Copy link, paste in Settings.
